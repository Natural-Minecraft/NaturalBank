...
public class NBConfigs {
    ...
    public void setupConfigs() {
        ...
        File file = new File(NaturalBank.INSTANCE().getDataFolder(), "saves.yml");
        if (!file.exists()) {
            try {
        ...
        savesConfig.options().header("DO NOT EDIT / REMOVE THIS FILE OR NaturalBank MAY GET RESET!");
    ...
    public void setupFile(String fileName, boolean backup) {
        ...
        if (backup) {
            File copyFile = new File(NaturalBank.INSTANCE().getDataFolder(), "backups" + File.separator + fileName);
            if (!copyFile.exists()) {
                try {
