...
import id.naturalsmp.naturalbank.listeners.playerChat.PlayerChatMethod;
...
public class NBEconomy {
    ...
    public NBEconomy(Bank originBank) {
     ...
     */
    public static NBEconomy get(String bankName) {
     ...
     */
    public static List<NBEconomy> list() {
        List<NBEconomy> economies = new ArrayList<>();
        for (Bank bank : BankRegistry.getBanks().values())
            economies.add(bank.getBankEconomy());
        return economies;
    }
     ...
     *
     * @return A hashmap with the player name as KEY and the sum of all the player
     *         bank balances as VALUE.
     */
    public static LinkedHashMap<String, BigDecimal> getAllEconomiesBankBalances() {
        ...
        for (OfflinePlayer p : Bukkit.getOfflinePlayers())
            balances.put(p.getName(), getBankBalancesSum(p));
        return balances;
    }
    ...
    public static BigDecimal getBankBalancesSum(OfflinePlayer p) {
        ...
        for (NBEconomy economy : list()) {
            BigDecimal add;
            if (economy.isPlayerLoaded(p))
                add = economy.getBankBalance(p);
            else
                add = economy.getHolder(p).money;
     ...
     *
     * @param p             The player.
     * @param wasRegistered If false, the money in the main bank will be set as
     *                      first join amount.
     * @return The holder of the specified player.
     */
    public Holder getHolder(OfflinePlayer p, boolean wasRegistered) {
        if (isPlayerLoaded(p))
            return holders.get(p.getUniqueId());
        ...
        holder.bankLevel = NBSQL.getBankLevel(p, bankName);
        holder.debt = NBSQL.getDebt(p, bankName);
        holder.money = useStartAmount ? ConfigValues.getStartAmount() : NBSQL.getMoney(p, bankName);
     ...
     * <p>
     * If the player hasn't been loaded or is offline, you should use getHolder to
    ...
    public BigDecimal getBankBalance(OfflinePlayer p) {
        ...
        if (p.isOnline() && !isPlayerLoaded(p))
            return BigDecimal.ZERO;

        // Using #getHolder fixes the problem with offline values, loading them if not
        // present in the hashmap, or getting the already registered for better
        // performance.
     ...
     * NBTransactionListeners without ignoring the events,
     * it will cause an infinite loop and crash the server.
     * <p>
     * To fix that, use the method #(OfflinePlayer, BigDecimal,
     ...
     * Set the player's money to the specified amount.
     *
     * @param ignoreEvents Choose if ignoring or not the NaturalBank transaction
     *                     event.
     * @return Number representing the actual amount set.
     */
    ...
    private BigDecimal setBankBalance(OfflinePlayer p, BigDecimal amount, boolean ignoreEvents, TransactionType type) {
        ...
        if (!startAndCheckOperationAllowed(p))
            return result;

        if (!ignoreEvents) {
            NBPreTransactionEvent event = preTransactionEvent(p, type, amount);
        ...
        if (!ignoreEvents)
            afterTransactionEvent(p, type, result);
        return result;
    }
    ...
    private BigDecimal removeBankBalance(OfflinePlayer p, BigDecimal amount, boolean ignoreEvents,
    ...
    /**
     * Method internally used to simplify the transactions, automatically end
     * operations.
     */
    ...
    /**
     * Shortcut for the deposit method of NaturalBank.
     *
    ...
    public void deposit(Player p, BigDecimal amount) {
        ...
        if (NBUtils.isBankFull(p, originBank))
            return;

        NBPreTransactionEvent event = preTransactionEvent(p, TransactionType.DEPOSIT, amount);
        if (event.isCancelled())
            return;

        amount = event.getTransactionAmount();
        if (minimumAmount(p, amount, ConfigValues.getDepositMinimumAmount()))
            return;

        Economy economy = NaturalBank.INSTANCE().getVaultEconomy();
        BigDecimal wallet = BigDecimal.valueOf(economy.getBalance(p));
        if (!NBUtils.checkPreRequisites(wallet, amount, p))
            return;
        ...
        BigDecimal maxDeposit = ConfigValues.getMaxDepositAmount(); // If it's more than the max deposit amount, use the
                                                                    // max amount.
        if (maxDeposit.compareTo(BigDecimal.ZERO) > 0 && amount.compareTo(maxDeposit) >= 0)
            amount = maxDeposit;

        BigDecimal capacity = BankUtils.getCapacity(originBank, p).subtract(getBankBalance(p)),
                finalAmount = amount.min(capacity);

        BigDecimal depositTaxes = ConfigValues.getDepositTaxes(), taxes = BigDecimal.ZERO;
        if (depositTaxes.compareTo(BigDecimal.ZERO) > 0 && !p.hasPermission("NaturalBank.deposit.bypass-taxes")) {
        ...
        if (NBUtils.hasFailed(p, depositResponse))
            return;

        addBankBalance(p, actualDepositingMoney, true);

        List<String> replacers = NBUtils.placeValues(p, actualDepositingMoney);
        replacers.addAll(NBUtils.placeValues(taxes, "taxes"));
        NBMessages.sendIdentifier(p, "Success-Deposit", replacers);

        if (ConfigValues.isDepositSoundEnabled())
            NBUtils.playSound(ConfigValues.getDepositSound(), p);

        afterTransactionEvent(p, TransactionType.DEPOSIT, amount);
    }

    /**
     * Shortcut for the withdraw method of NaturalBank.
     *
    ...
    public void withdraw(Player p, BigDecimal amount) {
        ...
        NBPreTransactionEvent event = preTransactionEvent(p, TransactionType.WITHDRAW, amount);
        if (event.isCancelled())
            return;

        amount = event.getTransactionAmount();
        if (minimumAmount(p, amount, ConfigValues.getWithdrawMinimumAmount()))
            return;

        BigDecimal bankBal = getBankBalance(p);
        if (!NBUtils.checkPreRequisites(bankBal, amount, p))
            return;

        BigDecimal maxWithdraw = ConfigValues.getMaxWithdrawAmount();
        if (maxWithdraw.compareTo(BigDecimal.ZERO) > 0 && amount.compareTo(maxWithdraw) >= 0)
            amount = maxWithdraw;
        if (bankBal.compareTo(amount) < 0)
            amount = bankBal;

        BigDecimal withdrawTaxes = ConfigValues.getWithdrawTaxes(), taxes = BigDecimal.ZERO;
        if (withdrawTaxes.compareTo(BigDecimal.ZERO) > 0 && !p.hasPermission("NaturalBank.withdraw.bypass-taxes")) {
        ...
        EconomyResponse withdrawResponse = NaturalBank.INSTANCE().getVaultEconomy().depositPlayer(p,
                amount.subtract(taxes).doubleValue());
        if (NBUtils.hasFailed(p, withdrawResponse))
            return;

        removeBankBalance(p, amount, true);

        List<String> replacers = NBUtils.placeValues(p, amount.subtract(taxes));
        replacers.addAll(NBUtils.placeValues(taxes, "taxes"));
        NBMessages.sendIdentifier(p, "Success-Withdraw", replacers);

        if (ConfigValues.isWithdrawSoundEnabled())
            NBUtils.playSound(ConfigValues.getWithdrawSound(), p);

        afterTransactionEvent(p, TransactionType.WITHDRAW, amount);
    }

    /**
     * Initialize the custom withdraw task for the selected player (Withdraw through
     * chat).
     *
     * @param p The player.
     */
    public void customDeposit(Player p) {
        if (MessageValues.isTitleCustomAmountEnabled())
            NBUtils.sendTitle(MessageValues.getCustomDepositTitle(), p);

        NBMessages.sendIdentifier(p, "Chat-Deposit");
        p.closeInventory();

        NBPlayer NBPlayer = PlayerRegistry.get(p);
        Bank bank = getOriginBank();
        NBPlayer.setOpenedBank(bank);
        NBPlayer.setDepositing(true);

        NBPlayer.setClosingTask(Bukkit.getScheduler().runTaskLater(NaturalBank.INSTANCE(), () -> {
            PlayerChatMethod.reopenBank(NBPlayer, bank.getBankGui());
            NBMessages.sendIdentifier(p, "Chat-Time-Expired");
        }, ConfigValues.getChatExitTime() * 20L));
    }

    /**
     * Initialize the custom deposit task for the selected player (Deposit through
     * chat).
     *
     * @param p The player.
     */
    public void customWithdraw(Player p) {
        if (MessageValues.isTitleCustomAmountEnabled())
            NBUtils.sendTitle(MessageValues.getCustomWithdrawTitle(), p);

        NBMessages.sendIdentifier(p, "Chat-Withdraw");
        p.closeInventory();

        NBPlayer NBPlayer = PlayerRegistry.get(p);
        Bank bank = getOriginBank();
        NBPlayer.setOpenedBank(bank);
        NBPlayer.setWithdrawing(true);
    ...
    public void pay(Player from, Player to, BigDecimal amount, Bank toBank) {
        if (!startAndCheckOperationAllowed(from) || !startAndCheckOperationAllowed(to))
            return;

        BigDecimal senderBalance = getBankBalance(from);
        // Check if the sender has at least more than 0 money
        if (senderBalance.compareTo(amount) < 0) {
            NBMessages.sendIdentifier(from, "Insufficient-Money");
            return;
        }

        NBEconomy toEconomy = toBank.getBankEconomy();
        ...
        if (toEconomy.getBankBalance(to).compareTo(BankUtils.getCapacity(toBank, to)) >= 0) {
            NBMessages.sendIdentifier(from, "Bank-Full", "%player%$" + to.getName());
            return;
        }

        BigDecimal added = toEconomy.addBankBalance(to, amount, TransactionType.PAY), extra = amount.subtract(added);
        NBMessages.sendIdentifier(to, "Payment-Received", NBUtils.placeValues(from, added));

        BigDecimal removed = removeBankBalance(from, amount.subtract(extra), TransactionType.PAY);
        NBMessages.sendIdentifier(from, "Payment-Sent", NBUtils.placeValues(to, removed));
    }

    private boolean minimumAmount(Player p, BigDecimal amount, BigDecimal minimum) {
        if (amount.compareTo(minimum) < 0) {
            NBMessages.sendIdentifier(p, "Minimum-Number", "%min%$" + minimum);
            return true;
        }
        return false;
    }

    private NBPreTransactionEvent preTransactionEvent(OfflinePlayer p, TransactionType type, BigDecimal amount) {
        NBPreTransactionEvent event = new NBPreTransactionEvent(
                p, type, getBankBalance(p), NaturalBank.INSTANCE().getVaultEconomy().getBalance(p), amount,
                originBank.getIdentifier());
        NBUtils.callEvent(event);
        return event;
    }

    private void afterTransactionEvent(OfflinePlayer p, TransactionType type, BigDecimal amount) {
        NBAfterTransactionEvent event = new NBAfterTransactionEvent(
                p, type, getBankBalance(p), NaturalBank.INSTANCE().getVaultEconomy().getBalance(p), amount,
                originBank.getIdentifier());
        NBUtils.callEvent(event);
    }
    ...
    private boolean startAndCheckOperationAllowed(OfflinePlayer p) {
        if ((p.isOnline() && !isPlayerLoaded(p)) || operations.contains(p.getUniqueId()))
            return false;
