...
public class BPEconomy {
     ...
     * Set the player's money to the specified amount.
     *
     * @param ignoreEvents Choose if ignoring or not the NaturalBank transaction event.
     * @return Number representing the actual amount set.
     */
    ...
    public void withdraw(Player p, BigDecimal amount) {
        ...
        EconomyResponse withdrawResponse = NaturalBank.INSTANCE().getVaultEconomy().depositPlayer(p, amount.subtract(taxes).doubleValue());
    ...
    public void customDeposit(Player p) {
        ...
        bpPlayer.setClosingTask(Bukkit.getScheduler().runTaskLater(NaturalBank.INSTANCE(), () -> {
    ...
    private BPPreTransactionEvent preTransactionEvent(OfflinePlayer p, TransactionType type, BigDecimal amount) {
        BPPreTransactionEvent event = new BPPreTransactionEvent(
                p, type, getBankBalance(p), NaturalBank.INSTANCE().getVaultEconomy().getBalance(p), amount, originBank.getIdentifier()
        );
