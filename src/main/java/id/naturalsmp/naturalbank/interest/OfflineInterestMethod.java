...
public class OfflineInterestMethod extends NBInterest.InterestMethod {

    @Override
    public void giveInterest(OfflinePlayer p) {
        if (NBInterest.InterestMethod.offlineTimeExpired(p))
            return;

        String offlinePermission = ConfigValues.getInterestOfflinePermission();
        if (!offlinePermission.isEmpty() && !NBUtils.hasOfflinePermission(p, offlinePermission))
            return;
        ...
        for (Bank bank : availableBanks) {
            ...
            // Since interest needs to be updated only when the player is offline to notify
            // it when joining, directly updated it from the database.
            // This method is already called asynchronously.
            NBSQL.setInterest(p, bank.getIdentifier(), NBSQL.getInterest(p, bank.getIdentifier()).add(added));
        }
    }
}
