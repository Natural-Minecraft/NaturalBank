...
public class MainCmd implements CommandExecutor, TabCompleter {
    ...
    public boolean onCommand(CommandSender s, Command command, String label, String[] args) {
        if (!ConfigValues.getWorldsBlacklist().isEmpty() && s instanceof Player p) {
            if (ConfigValues.getWorldsBlacklist().contains(p.getWorld().getName()) && !p.hasPermission("NaturalBank.worlds.blacklist.bypass")) {
        ...
        if (args.length == 0) {
            if (!BPUtils.hasPermission(s, "NaturalBank.use")) return true;
