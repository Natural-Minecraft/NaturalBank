...
/**
 * This class is used to create different instances of the bank list gui based
 * on the player-accessible banks.
...
public class BankListGui extends BankGui {
    ...
    public void openBankGui(Player p, boolean bypass) {
        ...
        Inventory bankListInventory = Bukkit.createInventory(new BankHolder(),
                MultipleBanksValues.getBankListGuiLines(), title);
        ...
        if (delay >= 0)
            player.setBankUpdatingTask(Bukkit.getScheduler().runTaskTimer(NaturalBank.INSTANCE(),
                    () -> updateBankGuiMeta(bankListInventory, p), delay, delay));
    ...
    public void placeContent(HashMap<Integer, NBGuiItem> items, Inventory bankInventory, Player p) {
        ...
        for (Bank bank : BankRegistry.getBanks().values()) {
            if (MultipleBanksValues.isShowNotAvailableBanks() && !BankUtils.isAvailable(bank, p))
                continue;

            BankGui gui = bank.getBankGui();
            NBGuiItem showBankItem = BankUtils.isAvailable(bank, p) ? gui.getAvailableBankListItem()
                    : gui.getUnavailableBankListItem();
